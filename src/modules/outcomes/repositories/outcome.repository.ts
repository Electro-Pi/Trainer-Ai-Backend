...
export class OutcomeRepository extends BaseRepository<Outcome, OutcomeDelegate> {
   ...
   * it yet — no learner assignment/progress, no session, no recommendation,
   * no rubric/question bank, and it isn't another content item's
   * prerequisite. Once any of those exist, deleting would either orphan real
   * training/assessment history or hit the FK constraint (none of these
   * relations cascade) — archive (`setEnabled(false)`) instead, per the
   * deactivate/archive-not-delete convention.
   */
